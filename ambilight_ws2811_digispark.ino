...
//  SerialUSB.begin(BAUDRATE);   // Init serial speed
  SerialUSB.begin();   // Init serial without BAUDRATE for CDC ACMx
...
void loop()
{
  switch(state)
  {
    case STATE_WAITING:                  // *** Waiting for prefix ***
      ...
      { 
        readSerial = SerialUSB.read();      // Read one character
    ...
    case STATE_DO_PREFIX:                // *** Processing Prefix ***
      if( SerialUSB.available() > sizeof(prefix) - 2 ) 
      {      
          SerialUSB.read( );
    ...
    case STATE_DO_DATA:                  // *** Process incoming color data ***
      if( SerialUSB.available() > 2 )       // if we receive more than 2 chars
      {
        SerialUSB.read();   // Abuse buffer to temp store 3 charaters
